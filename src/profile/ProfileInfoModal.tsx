...
import type { AvatarKey } from './avatarCatalog';
...
export default function ProfileInfoModal({ visible, token, initialName, initialEmail, close, saveName, onAvatarChanged, embedded = false }: {
  ...
  const selectAvatar = async (avatarKey: AvatarKey) => {
    if (avatarSaving) return;
    setAvatarSaving(true);
    try {
      const response = await apiRequest<ProfileResponse>('/auth/profile/avatar', {
        method: 'POST',
        token,
      ...
      showNotice({ tone: 'success', title: 'Avatarın güncellendi', message: 'Yeni avatarın profilinde, ilanlarında, mesajlarında ve sıralamada görünecek.' });
    } catch (error) {
      showNotice({ tone: 'error', title: 'Avatar güncellenemedi', message: error instanceof ApiError ? error.message : 'Avatar seçimi kaydedilemedi. Yeniden deneyebilirsin.' });
    } finally {
