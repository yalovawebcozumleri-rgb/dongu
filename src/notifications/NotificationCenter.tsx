      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable onPress={() => void open(item)} style={({ pressed }) => [x.row, !item.read && x.rowUnread, pressed && x.pressed]}>
            ...
            <View style={x.rowActions}>
              <Text style={x.arrow}>{String.fromCharCode(8250)}</Text>
              {!item.type.startsWith('admin_') && (
                <Pressable accessibilityRole="button" accessibilityLabel="Bildirimi sil" onPress={event => { event.stopPropagation(); void removeNotification(item); }} style={x.deleteButton}>
                  <Text style={x.deleteButtonText}>Sil</Text>
                </Pressable>
              )}
            </View>
          </Pressable>
