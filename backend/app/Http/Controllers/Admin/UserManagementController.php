...
class UserManagementController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $users = $query->latest('id')->paginate($perPage)->withQueryString()->through(fn (User $user) => [
            ...
            'restriction_ends_at' => $this->currentRestriction($user)?->ends_at?->toIso8601String(),
    ...
    public function show(User $user): Response
    {
        ...
        return Inertia::render('Admin/Users/Show', [
            'profile' => [
                ...
                'completed_transactions' => $user->completed_transactions,
                'created_at' => $user->created_at?->toIso8601String(),
                'listings' => $user->listings->map(fn (Listing $listing) => ['id' => $listing->id, 'status' => $listing->status, 'public_area' => $listing->public_area, 'published_at' => $listing->published_at?->toIso8601String()]),
                'sanctions' => $user->moderationSanctions->map(fn (ModerationSanction $sanction) => [
                    ...
                    'revoked_at' => $sanction->revoked_at?->toIso8601String(),
