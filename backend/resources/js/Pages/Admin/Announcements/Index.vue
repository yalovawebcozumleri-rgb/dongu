...
const openComposer = () => { composeOpen.value = true; };
        ...
        <div v-if="campaigns.data.length" class="overflow-x-auto">
          <table class="w-full min-w-[960px] text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
              <tr v-for="campaign in campaigns.data" :key="campaign.id" class="text-slate-800 transition hover:bg-slate-50/80">
                <td class="max-w-[430px] px-5 py-4">
                  <p class="text-xs font-semibold uppercase tracking-wide text-emerald-800">#{{ campaign.id }} · {{ types[campaign.type] }}</p>
        ...
        <div class="overflow-y-auto p-6">
          ...
          <section class="mt-6 border-t border-slate-200 pt-5">
            ...
            <dl class="mt-4 grid gap-x-8 sm:grid-cols-2">
              ...
              <div class="border-b border-slate-100 py-3"><dt class="text-xs font-medium text-slate-600">Tekrar bitişi</dt><dd class="mt-1 text-sm font-semibold text-slate-950">{{ $adminDate(viewingCampaign.ends_at, 'Tek gönderim') }}</dd></div>
    ...
    <div v-if="composeOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm" @click.self="closeComposer" @keydown.esc.window="closeComposer">
      <section role="dialog" aria-modal="true" aria-labelledby="announcement-modal-title" class="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
        <div class="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
                  <div><h2 id="announcement-modal-title" class="text-lg font-semibold text-slate-950">Yeni duyuru oluştur</h2><p class="mt-1 text-sm text-slate-600">Mesajı taslak olarak kaydet, ileri bir zamana planla veya hemen gönder.</p></div>
