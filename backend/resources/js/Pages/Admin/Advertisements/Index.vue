...
const form = useForm({
  sponsorName: '', headline: '', body: '', format: 'banner', image: null,
  ctaLabel: '', targetUrl: '', startsAt: '', endsAt: '',
  priority: 0, isActive: false, androidEnabled: true, iosEnabled: true, placements: [],
});
...
const formatLabels = { banner: '2:1 sponsorlu banner' };
...
const applyFilters = () => router.get('/admin/advertisements/sponsors', {
  ...
  <AdminLayout eyebrow="Reklamlar" title="Sponsorlu bannerlar" description="Marka kampanyalarını, yayın alanlarını, platformları ve performans ölçümlerini AdMob sisteminden bağımsız yönet.">
    <main class="mx-auto max-w-[1600px] px-5 py-8 lg:px-8">
      ...
      <section class="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-5"><p class="text-sm font-semibold text-slate-700">Toplam gösterim</p><p class="mt-2 text-3xl font-semibold text-slate-950">{{ Number(counts.impressions).toLocaleString('tr-TR') }}</p><p class="mt-1 text-xs text-slate-600">Sponsorlu banner kampanyaları</p></article>
      ...
      <section class="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div v-if="campaigns.data.length" class="overflow-x-auto">
          <table class="w-full min-w-[1180px] text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100"><tr v-for="campaign in campaigns.data" :key="campaign.id" class="text-slate-800 transition hover:bg-slate-50/80">
              <td class="max-w-[330px] px-5 py-4"><p class="text-xs font-semibold uppercase tracking-wide text-emerald-800">#{{ campaign.id }} · {{ campaign.sponsorName }}</p><p class="mt-1 truncate font-semibold text-slate-950">{{ campaign.headline }}</p><p class="mt-1 text-xs text-slate-600">{{ formatLabels[campaign.format] }}</p><div class="mt-2 flex gap-1.5"><span v-if="campaign.androidEnabled" class="rounded bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-800">Android</span><span v-if="campaign.iosEnabled" class="rounded bg-sky-50 px-2 py-0.5 text-[11px] font-semibold text-sky-800">iOS</span></div></td>
        ...
        <div v-else class="px-5 py-16 text-center"><p class="font-semibold text-slate-900">Bu filtrelerde sponsorlu banner kampanyası bulunamadı</p><p class="mt-1 text-sm text-slate-600">Uygun kampanya yoksa mobil uygulamada banner alanı ve boşluk gösterilmez.</p><button type="button" class="mt-4 text-sm font-semibold text-emerald-700" @click="openComposer">İlk sponsorlu bannerı oluştur</button></div>
      </section>
      ...
      <section role="dialog" aria-modal="true" aria-labelledby="campaign-create-title" class="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
        ...
        <form class="overflow-y-auto p-6" @submit.prevent="submit">
          ...
          <div class="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <label class="text-xs font-semibold text-slate-700">Reklam biçimi <span class="mt-1.5 flex h-11 w-full items-center rounded-xl border border-slate-300 bg-slate-50 px-3 text-sm text-slate-950">2:1 sponsorlu banner</span></label>
            ...
            <label class="rounded-xl border border-dashed border-slate-300 p-4 text-xs font-semibold text-slate-700 md:col-span-2">Banner görseli <span class="text-red-600">*</span><span class="mt-1 block font-normal text-slate-600">Tam 2:1 oran · önerilen 1200 × 600 px · JPG, PNG veya WebP · en fazla 4 MB</span><input required type="file" accept="image/jpeg,image/png,image/webp" class="mt-3 block w-full text-sm" @change="form.image = $event.target.files[0]" /><span v-if="form.errors.image" class="mt-2 block text-red-700">{{ form.errors.image }}</span></label>
            ...
            <label class="text-xs font-semibold text-slate-700 md:col-span-2">Yönlendirme bağlantısı<input v-model="form.targetUrl" type="url" maxlength="500" placeholder="https://" class="mt-1.5 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm text-slate-950" /><span v-if="form.errors.targetUrl" class="mt-1 block text-xs text-red-700">{{ form.errors.targetUrl }}</span></label><fieldset class="rounded-xl border border-slate-200 p-4 md:col-span-2"><legend class="px-1 text-xs font-semibold text-slate-700">Yayın platformları <span class="text-red-600">*</span></legend><div class="mt-2 grid gap-3 sm:grid-cols-2"><label class="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-800"><input v-model="form.androidEnabled" type="checkbox" class="size-5" /> Android</label><label class="flex items-center gap-3 rounded-xl bg-sky-50 px-4 py-3 text-sm font-semibold text-slate-800"><input v-model="form.iosEnabled" type="checkbox" class="size-5" /> iOS</label></div><p v-if="form.errors.androidEnabled || form.errors.iosEnabled" class="mt-2 text-xs font-semibold text-red-700">{{ form.errors.androidEnabled || form.errors.iosEnabled }}</p></fieldset>
            ...
            <label class="text-xs font-semibold text-slate-700">Öncelik <span class="text-red-600">*</span><input v-model.number="form.priority" type="number" min="0" max="1000" class="mt-1.5 h-11 w-full rounded-xl border border-slate-300 px-3 text-sm text-slate-950" /><span class="mt-1 block font-normal text-slate-500">Yüksek öncelikli kampanyalar dönüş havuzunda önce değerlendirilir.</span></label>
