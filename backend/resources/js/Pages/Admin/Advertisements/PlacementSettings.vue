...
const errors = ref({});
const form = reactive({ enabled: false, sourceOrder: [], firstAfter: 0, repeatEvery: 0, maxPerSession: 1, minItems: 0, adMobAndroidUnitId: '', adMobIosUnitId: '', boostHours: 24, dailyLimit: 3, ordinalsText: '2, 4' });
const pageSettings = computed(() => props.settings.filter(item => item.kind === 'native'));
const fixedSettings = computed(() => props.settings.filter(item => item.kind === 'house'));
const actionSettings = computed(() => props.settings.filter(item => ['interstitial', 'rewarded'].includes(item.kind)));
const sourceLabels = { direct: 'Kurumsal', admob: 'AdMob', house: 'Döngü' };
...
const open = item => {
  ...
  Object.assign(form, { enabled: item.enabled, sourceOrder: [...item.sourceOrder], firstAfter: item.firstAfter, repeatEvery: item.repeatEvery, maxPerSession: item.maxPerSession, minItems: item.minItems, adMobAndroidUnitId: item.adMobAndroidUnitId || '', adMobIosUnitId: item.adMobIosUnitId || '', boostHours: item.boostHours, dailyLimit: item.dailyLimit, ordinalsText: (item.ordinals || []).join(', ') });
};
...
const save = () => {
  ...
  pending.value = true; errors.value = {};
  const ordinals = form.ordinalsText.split(',').map(value => Number(value.trim())).filter(value => Number.isInteger(value) && value > 0);
  router.patch(`/admin/advertisement-placements/${editing.value.id}`, { ...form, ordinals }, { preserveScroll: true, onSuccess: () => { editing.value = null; }, onError: value => { errors.value = value; }, onFinish: () => { pending.value = false; } });
};
</script>

<template>
  <section class="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <header class="border-b border-slate-200 px-5 py-4"><h2 class="text-lg font-semibold text-slate-950">Reklam alanları</h2><p class="mt-1 text-sm text-slate-600">Uygulamadaki standart reklam kartlarını, kaynak sırasını ve gösterim yoğunluğunu tek yerden yönet.</p></header>
  ...
  <section class="mt-5 grid gap-4 lg:grid-cols-2">
    <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 class="text-lg font-semibold text-slate-950">Sabit Döngü reklamları</h2><p class="mt-1 text-sm text-slate-600">Bu alanlarda AdMob veya kurumsal kampanya gösterilmez.</p><div class="mt-4 divide-y divide-slate-100"><div v-for="item in fixedSettings" :key="item.key" class="flex items-center justify-between py-3"><div><p class="font-semibold text-slate-950">{{ item.label }}</p><p class="text-xs text-slate-600">Yalnızca Döngü reklamı</p></div><span class="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-800">Her zaman açık</span></div></div></article>
    <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 class="text-lg font-semibold text-slate-950">Sayfa dışı reklamlar</h2><p class="mt-1 text-sm text-slate-600">Geçiş ve ödüllü video kurallarını yönet.</p><div class="mt-4 divide-y divide-slate-100"><div v-for="item in actionSettings" :key="item.key" class="flex items-center justify-between gap-4 py-3"><div><p class="font-semibold text-slate-950">{{ item.label }}</p><p class="text-xs text-slate-600">{{ displayRule(item) }}</p></div><div class="flex items-center gap-2"><span :class="['rounded-full px-2.5 py-1 text-xs font-semibold', item.enabled ? 'bg-emerald-50 text-emerald-800' : 'bg-slate-100 text-slate-700']">{{ item.enabled ? 'Açık' : 'Kapalı' }}</span><button class="rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold" @click="open(item)">Düzenle</button></div></div></div></article>
  </section>

  <div v-if="editing" class="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm" @click.self="close" @keydown.esc.window="close"><section class="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl"><header class="border-b border-slate-200 px-6 py-5"><h2 class="text-xl font-semibold text-slate-950">{{ editing.label }}</h2><p class="mt-1 text-sm text-slate-600">Değişiklikler kaydedildiğinde mobil uygulamaya doğrudan uygulanır.</p></header><form class="overflow-y-auto p-6" @submit.prevent="save">
    ...
    <template v-if="editing.kind === 'native'"><fieldset class="mt-5"><legend class="text-sm font-semibold text-slate-950">Reklam kaynakları ve sırası</legend><div class="mt-2 space-y-2"><div v-for="(source, index) in form.sourceOrder" :key="source" class="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3"><span class="text-sm font-semibold">{{ index + 1 }}. {{ sourceLabels[source] }}</span><div class="flex gap-1"><button type="button" class="rounded-lg border px-2 py-1" @click="move(index, -1)">↑</button><button type="button" class="rounded-lg border px-2 py-1" @click="move(index, 1)">↓</button><button type="button" class="rounded-lg border px-2 py-1 text-red-700" @click="toggleSource(source)">Kaldır</button></div></div></div><div class="mt-2 flex gap-2"><button v-for="source in ['direct','admob','house'].filter(value => !form.sourceOrder.includes(value))" :key="source" type="button" class="rounded-lg border border-emerald-300 px-3 py-2 text-xs font-semibold text-emerald-800" @click="toggleSource(source)">+ {{ sourceLabels[source] }}</button></div></fieldset>
